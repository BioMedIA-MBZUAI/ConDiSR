...
class SimpleNet(nn.Module):
    ...
    def __init__(self, cfg, model_cfg, num_classes, **kwargs):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(256, 128, 4, stride=2, padding=1),  # [batch, 128, 12, 12]
            nn.ReLU(),
			nn.ConvTranspose2d(128, 64, 4, stride=2, padding=1),  # [batch, 64, 24, 24]
            nn.ReLU(),
			nn.ConvTranspose2d(64, 32, 4, stride=2, padding=1),  # [batch, 32, 48, 48]
            nn.ReLU(),
            nn.ConvTranspose2d(32, 1, 4, stride=2, padding=1),   # [batch, 1, 96, 96]
            nn.Sigmoid(),
        )
        ...
        # self.dom_discr = nn.Linear(fdim // 2, len(cfg.DATASET.SOURCE_DOMAINS))
    ...
    def forward(self, x, return_feature=False):
        ...
        y = self.backbone.layer4(f)
        y = self.backbone.global_avgpool(y)
        y = self.fc(y.squeeze())
...
class FDFreq(TrainerX):
    """FDNaive model.
    
    A.k.a. Feature Disentanglement Naive
    """

    def forward_backward(self, batch):
        input, target, high_gt = self.parse_batch_train(batch)
        
        y_out, high_out = self.model(input)
        loss_l = F.cross_entropy(y_out, target)
        loss_r = F.mse_loss(high_out, high_gt)
        
        loss = loss_l + loss_r
        self.model_backward_and_update(loss)

        loss_summary = {
            "loss_label": loss_l.item(),
            "loss_reconstruction": loss_r.item(),
            "acc": compute_accuracy(y_out, target)[0].item(),
        }
