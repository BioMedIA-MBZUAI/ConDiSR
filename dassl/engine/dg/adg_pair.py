...
class SimpleNet(nn.Module):
    ...
    def __init__(self, cfg, model_cfg, num_classes, **kwargs):
        ...
        # self.d_in1 = nn.Linear(fdim, fdim)
        # self.d_in2 = nn.Linear(fdim, fdim)
        # self.d_mid1 = nn.Linear(fdim * 2, fdim)
        # self.d_mid2 = nn.Linear(fdim, fdim)
        # self.d_out = nn.Linear(fdim, 1)
    ...
    def forward(self, x, return_feature=False):
        ...
        # d1 = self.d_in1(self.rev_grad(f1))
        # d2 = self.d_in2(self.rev_grad(f2))
        # dm = self.d_mid1(F.relu(torch.cat([d1, d2], 1)))
        # dm = self.d_mid2(F.relu(dm))
        # d_score = self.d_out(F.relu(dm))
...
class ADGPair(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        out_l, out_d = self.model(input)
        loss_l = F.cross_entropy(out_l, target)

        out_d = F.pad(out_d.unsqueeze(1), (0,0,0,1), mode='replicate')

        loss_d = self.con_crit(out_d, labels=dom)


        loss = loss_l + loss_d * 0.0001
        self.model_backward_and_update(loss)

        loss_summary = {
            "loss_l": loss_l.item(),
            "loss_d": loss_d.item(),
            "acc": compute_accuracy(out_l, target)[0].item(),
        }
