...
class SimpleNet(nn.Module):
    ...
    def __init__(self, size=48, c=64):
        ...
        self.decoder = nn.Sequential(
            nn.Conv3d(1, 4, (1,4,4), stride=(1,2,2), padding=(0,1,1)), # 24
            nn.ReLU(),
            nn.Conv3d(4, 16, (1,4,4), stride=(1,2,2), padding=(0,1,1)), # 12
            nn.ReLU(),
            nn.Conv3d(16, 32, (1,4,4), stride=(1,2,2), padding=(0,1,1)), # 6
            nn.ReLU(),
            nn.Conv3d(32, 64, (1,4,4), stride=(1,2,2), padding=(0,1,1)), # 3
        )
        self.pooler = nn.AdaptiveAvgPool3d(1)
        
        # self.decoder = nn.Sequential(
        #     nn.Conv2d(1, 4, 4, stride=2, padding=1), # 24
        #     nn.ReLU(),
        #     nn.Conv2d(4, 16, 4, stride=2, padding=1), # 12
        #     nn.ReLU(),
        #     nn.Conv2d(16, 32, 4, stride=2, padding=1), # 6
        #     nn.ReLU(),
        #     nn.Conv2d(32, 64, 4, stride=2, padding=1), # 3
        # )
...
class UNetCCSDG(nn.Module):
    def __init__(self, resnet='resnet18', num_classes=2, pretrained=False, cutoff=1):
        super().__init__()
        cut, lr_cut = [8, 6]
        ...
        first_layers = layers[0:cutoff]
        other_layers = layers[cutoff:]
        base_layers = nn.Sequential(*other_layers)
        self.first_layers = nn.Sequential(*first_layers)
        self.rn = base_layers

        if (cutoff == 1) or (cutoff == 5):
            c = 64
        else:
            c = 2**(cutoff + 1)
        self.channel_prompt = nn.Parameter(torch.randn(2, c, 1, 1))
    ...
    def forward_first_layers(self, x, tau=0.1):
        x = self.first_layers(x)  # 8 64 256 256
    ...
    def forward_rn(self, fmap):
        f_deep = self.rn(fmap)
        output = self.pooler(f_deep)
        output = output.view(output.size(0), -1)
        output = self.classifier(output)
        
        return output

    def forward(self, x, tau=0.1, return_deep=False):
        x = self.first_layers(x)  # 8 64 256 256
        ...
        f_deep = self.rn(f_content)
        output = self.pooler(f_deep)
        output = output.view(output.size(0), -1)
        output = self.classifier(output)
        
        if return_deep:
            return output, f_deep
        else:
            return output
...
class SimpleTrainer(TrainerBase):
    ...
    def build_model(self):
        ...
        self.model = UNetCCSDG(resnet='resnet18', num_classes=self.num_classes, pretrained=True, cutoff=cfg.CCSDG.CUTOFF)
...
class CCSDGAdv(TrainerX):
    """CCSDG Adversarial model.
    ...
    def forward_backward(self, batch):
        ...
        loss = content_loss + style_loss + loss_fda + loss_gla + loss_base
        
        str_idxs = np.where(self.model.channel_prompt.detach().squeeze().argmax(0).cpu().numpy() == 0)[0]
        sty_idxs = np.where(self.model.channel_prompt.detach().squeeze().argmax(0).cpu().numpy() == 1)[0]
        # if len(str_idxs) > 0:
        #     str_c = f_content.detach()[:,str_idxs,:,:].unsqueeze(1)
        #     str_c_fda = f_content_fda.detach()[:,str_idxs,:,:].unsqueeze(1)
        #     str_c_GLA = f_content_GLA.detach()[:,str_idxs,:,:].unsqueeze(1)
            
        #     str_c_out = self.autoenc(str_c)
        #     str_c_fda_out = self.autoenc(str_c_fda)
        #     str_c_GLA_out = self.autoenc(str_c_GLA)
            
        #     loss_base_c = F.cross_entropy(str_c_out, target)
        #     loss_fda_c = F.cross_entropy(str_c_fda_out, target)
        #     loss_gla_c = F.cross_entropy(str_c_GLA_out, target)
        #     loss_c = loss_base_c + loss_fda_c + loss_gla_c
        ...
        if len(sty_idxs) > 0:
            sty_c = rg(f_style)[:,sty_idxs,:,:].unsqueeze(1)
            sty_c_fda = rg(f_style_fda)[:,sty_idxs,:,:].unsqueeze(1)
            sty_c_GLA = rg(f_style_GLA)[:,sty_idxs,:,:].unsqueeze(1)
            
            sty_c_out = self.autoenc(sty_c)
            sty_c_fda_out = self.autoenc(sty_c_fda)
            sty_c_GLA_out = self.autoenc(sty_c_GLA)
            
            loss_base_c_y = F.cross_entropy(sty_c_out, target)
            loss_fda_c_y = F.cross_entropy(sty_c_fda_out, target)
            loss_gla_c_y = F.cross_entropy(sty_c_GLA_out, target)

            loss_c_y = loss_base_c_y + loss_fda_c_y + loss_gla_c_y

            loss += loss_c_y * 1.0
        
        self.model_backward_and_update(loss)

        sty_idx = np.random.choice(sty_idxs)
        sty_c = f_style.detach()[:,sty_idxs,:,:].unsqueeze(1)
        with torch.no_grad():
            sty_c_out = self.autoenc(sty_c)
        ...
        loss_summary = {
            ...
            # "loss_c" : loss_c.item(),
            "loss_c_y" : loss_c_y.item(),
            "acc": compute_accuracy(out_base, target)[0].item(),
            # "acc_c_str": compute_accuracy(str_c_out, target)[0].item(),
            "acc_c_sty": compute_accuracy(sty_c_out, target)[0].item(),
        }
