...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output, _, __ = self.model_inference(input)
...
class FDNaive(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        out_l, out_d, feat = self.model(input)
        loss_l = F.cross_entropy(out_l, target)
        loss_d = F.cross_entropy(out_d, dom)
        cm = torch.cov(feat)
        
        m1 = cm[b_2:, :b_2] ** 2
        m2 = cm[:b_2, b_2:] ** 2
        
        loss_cov = m1.sum() + m2.sum()
        
        loss = loss_l + loss_d + loss_cov * 0.001
