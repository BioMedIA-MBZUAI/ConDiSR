...
class SimpleNet(nn.Module):
    ...
    def __init__(self, cfg, model_cfg, num_classes, **kwargs):
        ...
        self.bb = timm.models.VisionTransformer(img_size=(96,96), patch_size=8, embed_dim=384)
        
        self.channel_prompt = nn.Parameter(torch.randn(2, 145))
    ...
    def forward_first_layers(self, x, tau=0.1):
        ...
        p = torch.softmax(self.channel_prompt / tau, 0)
        f_str = f * p[0].unsqueeze(1)
        f_sty = f * p[1].unsqueeze(1)
        
        return f_str, f_sty
    
    def forward_rn(self, f):
        out = self.bb.forward_head(f)
...
class SimpleTrainer(TrainerBase):
    ...
    def build_model(self):
        ...
        self.model = SimpleNet(cfg, cfg.MODEL, 2)
    ...
    def model_inference(self, input):
        aux = self.model.forward_first_layers(input)[0]
        return self.model.forward_rn(aux)
...
class ViTVanilla(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        f_content_p = f_content.mean(1)
        f_style_p = f_style.mean(1)
        f_content_fda_p = f_content_fda.mean(1)
        f_style_fda_p = f_style_fda.mean(1)
        f_content_GLA_p = f_content_GLA.mean(1)
        f_style_GLA_p = f_style_GLA.mean(1)
        ...
        out_base = self.model.forward_rn(f_content)
        out_fda = self.model.forward_rn(f_content_fda)
        out_gla = self.model.forward_rn(f_content_GLA)
